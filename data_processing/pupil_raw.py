...
def process_raw_pupil(yaml_path: Path, pkl_path: Path):
    emotion_dialations = {}
        ...
        for line in f:
            if match := re.search(".*MID:(?P<MID>\d+),.*", line):
                ...
                if curr_id in ID_TO_EMOTION:
                    emotion_dialations[ID_TO_EMOTION[curr_id]].append((curr_lpd+float(match["RPD"]))/2)
    
    with open(pkl_path, 'wb') as f:
        pickle.dump(emotion_dialations, f)

def load_raw_pupil(pkl_path: Path) -> Dict[str, List[Tuple]]:
    with open(pkl_path, 'rb') as f:
        data = pickle.load(f)
    return data

if __name__ == "__main__":
    process_raw_pupil(Path(sys.argv[1]), Path(sys.argv[2]))
    #print(load_raw_pupil(Path(sys.argv[2])))
